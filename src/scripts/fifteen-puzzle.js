...
let move_counts =
  JSON.parse(localStorage.getItem("move_counts" + side_len)) || [];
...
function calculate_avg(some_times) {
  ...
  middle_times = times_copy
    .sort((a, b) => a - b)
    .slice(cut_size, len - cut_size);
...
function update_bests() {
  if (times.length > 0) {
    ...
    document.getElementById("best-single").innerHTML =
      `Best time: ${best_single_str}`;
  } else {
  ...
  if (times.length >= 3) {
    ...
    document.getElementById("session-average").innerHTML =
      `Session average: ${session_avg_str}`;
  } else {
    document.getElementById("session-average").innerHTML =
      "Session average: - ";
  }
  if (times.length >= avg_len) {
    ...
    document.getElementById("best-average").innerHTML =
      `Best ao${avg_len}: ${best_avg}`;
  } else {
...
function change_move_count_mps() {
  ...
  localStorage.setItem(
    "display_move_count",
    JSON.stringify(display_move_count),
  );
  draw_time_list();
}
...
function move_custom_focus(ui_or_puzzle) {
  if (ui_or_puzzle == "puzzle") {
    ...
    throw new Error(
      "move_custom_focus function takes only 'ui' or 'puzzle' as parameters",
    );
  }
}
